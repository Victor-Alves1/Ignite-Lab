      ...
      if (node.operation === "subscription") {
        ...
        if (subscriptionType) {
          ...
          if (fields.size > 1) {
            ...
            context.reportError(new GraphQLError(operationName2 != null ? `Subscription "${operationName2}" must select only one top level field.` : "Anonymous Subscription must select only one top level field.", {
          ...
          for (const fieldNodes of fields.values()) {
            ...
            if (fieldName.startsWith("__")) {
              context.reportError(new GraphQLError(operationName2 != null ? `Subscription "${operationName2}" must not select an introspection top level field.` : "Anonymous Subscription must not select an introspection top level field.", {
                nodes: fieldNodes
              }));
            }
          }
    ...
    OperationDefinition(node) {
      ...
      if (operationName2) {
        if (knownOperationNames[operationName2.value]) {
          context.reportError(new GraphQLError(`There can be only one operation named "${operationName2.value}".`, {
            nodes: [
